...
Discuz! 포럼(색화당 등)이나 일부 토렌트 사이트는 동일 게시판 내에서 특정 서브 카테고리(`typeid` 등)를 선택할 경우 정적 URL 구조가 아닌 동적 쿼리스트링 구조로 변경됩니다. Feeder는 이를 플랫폼 차원에서 지원합니다.

* **수집 테스트 창에서 입력:**
  * 기본 게시판 테스트: `166` (기본 `BOARD_URL_RULE`로 조합)
  * 서브 카테고리 테스트: `166_875` 또는 `166:875` 입력 후 엔터 (플랫폼이 게시판 ID `166`과 서브 카테고리 `875`를 자동 분리하여 `SUBCAT_URL_RULE`로 조합)
* **스케줄링 등록 시:**
  * `게시판 추가/수정` 모달에서 `게시판 ID`(`166`)와 `서브 카테고리 ID`(`875`)를 각각 입력하여 개별 등록 가능.
* **DB 격리 및 독립 피드 발행:**
  * 서브 카테고리가 지정된 게시판은 내부적으로 `166_875`라는 고유 복합 키로 관리되어, 전체 게시판 수집 데이터와 중복되지 않고 독립된 RSS 피드로 발행됩니다.
